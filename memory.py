...
Experience = namedtuple('Experience', 'state0, action, reward, terminal1, error, state1')
    ...
    def sample(self, batch_size, batch_idxs=None):
        ...
        for idx in batch_idxs:
            ...
            experiences.append(Experience(state0=state0, action=action, reward=reward,
                                          state1=state1, terminal1=terminal1, error = None))
...
class PER:  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def append(self, observation, action, reward, terminal, error, next_state):
        sample = (observation, action, reward, terminal, error, next_state)
    ...
    def sample(self, n):
        ...
        for i in range(n):
            ...
            priorities.append(p)
            batch.append(Experience(state0=data[0], action=data[1], reward = data[2], terminal1 = data[3], error = data[4], state1=data[5]))
            idxs.append(idx)
    ...
    def update(self, idxs, errors):
        _ = [self.tree.update(idx, self._get_priority(error)) for (error, idx) in zip(errors, idxs)]
